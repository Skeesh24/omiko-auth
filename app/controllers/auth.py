...
from ..classes.dependencies import get_caching_service, get_users
...
auth_router = APIRouter(
    prefix="/auth", tags=["auth"], dependencies=[Depends(get_caching_service)]
)
...
async def refresh(
    ...
    recieved_token = Auhthorize.get_raw_jwt()
    if token != recieved_token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="received invalid token"
        )

    access_token = Auhthorize.create_access_token(username)
